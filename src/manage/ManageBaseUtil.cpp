...
INT32		CManageBaseUtil::GetPktPolicy(PDB_POLICY pDP, MemToken& RecvToken)
{
	...
	if(!RecvToken.TokenDel_32(pDP->nPInAcDocID))		goto INVALID_PKT;
	if(!RecvToken.TokenDel_32(pDP->nPInAcSfID))			goto INVALID_PKT;
	if(!RecvToken.TokenDel_32(pDP->nPInAcFileID))		goto INVALID_PKT;
...
INT32		CManageBaseUtil::SetPktPolicy(PDB_POLICY pDP, MemToken& SendToken)
{
	...
	SendToken.TokenAdd_32(pDP->nPInAcDocID);
	SendToken.TokenAdd_32(pDP->nPInAcSfID);
	SendToken.TokenAdd_32(pDP->nPInAcFileID);
