...
//		data.strObjectPath = _strlwr(data.strObjectPath.c_str());
	...
	PDB_LOG_RS_BK pdata = t_ManageLogRsBk->GetExistLogRsBk(data);
	if(pdata)
	{		
		if(_stricmp(pdata->strOrFileHash.c_str(), data.strOrFileHash.c_str()) == 0)
		{
	...
	{
		TListID tIDList;
		t_ManageLogRsBk->GetOldRsBk(&data, tIDList, pdata_bk->nFileKeppNum);
