...
INT32		CManageEnvLocal::LoadDBMS()
{
	...
	{		
		AddItem(*begin);
		AddKeySubIDMap(begin->nChkType, begin->nChkCode, begin->nID);
	}
    return 0;
}
//---------------------------------------------------------------------------
...
INT32					CManageEnvLocal::AddEnvLocal(DB_ENV_LOCAL&	data)
{
    ...
    AddItem(data);
	AddKeySubIDMap(data.nChkType, data.nChkCode, data.nID);
...
INT32					CManageEnvLocal::DelEnvLocal(UINT32 nID)
{
	...
	DelKeySubIDMap(pdata->nChkType, pdata->nChkCode);
    DeleteItem(nID);
    return 0;
}
//---------------------------------------------------------------------------
...
{
	PDB_ENV_LOCAL pdata = FindKeySubIDMapItem(nChkType, nChkCode);
	...
	AddEnvLocal(data);
	pdata = FindKeySubIDMapItem(nChkType, nChkCode);
