...
INT32		CManageSiteFile::LoadDBMS()
{
    ...
    for(begin; begin != end; begin++)
    {
		...
		if(begin->strFeKey.empty() == FALSE)		AddPosStrKeyID(begin->strFeKey, begin->nID, SS_SITE_FILE_KEY_TYPE_FE);
		if(begin->strFileHash.empty() == FALSE)	AddPosStrKeyID(begin->strFileHash, begin->nID, SS_SITE_FILE_KEY_TYPE_HASH);
    }
    return 0;
}
//---------------------------------------------------------------------------
...
INT32					CManageSiteFile::AddSiteFile(DB_SITE_FILE&	dsf)
{
	...
	if(dsf.strFeKey.empty() == FALSE)			AddPosStrKeyID(dsf.strFeKey, dsf.nID, SS_SITE_FILE_KEY_TYPE_FE);
	if(dsf.strFileHash.empty() == FALSE)		AddPosStrKeyID(dsf.strFileHash, dsf.nID, SS_SITE_FILE_KEY_TYPE_HASH);
...
INT32					CManageSiteFile::DelSiteFile(UINT32 nID)
{
	...
	if(pdsf->strFeKey.empty() == FALSE)		DelPosStrKeyID(pdsf->strFeKey, SS_SITE_FILE_KEY_TYPE_FE);
	if(pdsf->strFileHash.empty() == FALSE)	DelPosStrKeyID(pdsf->strFileHash, SS_SITE_FILE_KEY_TYPE_HASH);
...
{
	return FindItem(FindStrKeyID(strName));
}
//---------------------------------------------------------------------------
