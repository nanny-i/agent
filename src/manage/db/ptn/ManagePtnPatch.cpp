...
INT32		CManagePtnPatch::LoadDBMS()
{
    ...
    for(begin; begin != end; begin++)
    {
		...
		if(begin->strVulnGuid.empty() == FALSE)
			AddPosStrKeyID(begin->strVulnGuid, begin->nID, POS_KEY_STR_POSITION_PTN_PATCH_GUID_MAP);
    }
    return 0;
}
//---------------------------------------------------------------------------
...
INT32					CManagePtnPatch::AddPtnPatch(DB_PTN_PATCH&	data)
{
	...
	if(data.strVulnGuid.empty() == FALSE)
		AddPosStrKeyID(data.strVulnGuid, data.nID, POS_KEY_STR_POSITION_PTN_PATCH_GUID_MAP);
    return 0;
}
//---------------------------------------------------------------------------
...
INT32					CManagePtnPatch::DelPtnPatch(UINT32 nID)
{
	...
	if(pdata->strVulnGuid.empty() == FALSE)
		DelPosStrKeyID(pdata->strVulnGuid, POS_KEY_STR_POSITION_PTN_PATCH_GUID_MAP);
...
{
	return FindItem(FindStrKeyID(strKey));
}
//---------------------------------------------------------------------------
