...
{	
...

	{
		...
		{
			PDB_SITE_FILE pdsf = t_ManageSiteFile->FindPosStrKeyItem(begin->strFeKey, SS_SITE_FILE_KEY_TYPE_FE);
			if(!pdsf)		continue;
...
void		CLogicMgrSiteFile::CheckSiteFile(PDB_SITE_FILE pdsf_src)
{
	...
	dsf = *pdsf_src;
	pdsf = t_ManageSiteFile->FindPosStrKeyItem(dsf.strFeKey, SS_SITE_FILE_KEY_TYPE_FE);
...
void		CLogicMgrSiteFile::SendPkt_Sync(INT32 nOnceMaxNum)
{
	...
	{
		nOnceNum = ((tSendList.size() - nSendNum) > nOnceMaxNum ? nOnceMaxNum : (tSendList.size() - nSendNum));
...
void		CLogicMgrSiteFile::SendPkt_ReSend(INT32 nOnceMaxNum)
{
	...
	{
		nOnceNum = ((tIDList.size() - nSendNum) > nOnceMaxNum ? nOnceMaxNum : (tIDList.size() - nSendNum));
